...
assert.equal(alphaPages.status, 200);
assert.match(await alphaPages.text(), /Top pages/);
const alphaSources = await request(`/sites/${alpha.id}/sources?period=30`, { headers: { cookie: adminCookie } });
assert.equal(alphaSources.status, 200);
assert.match(await alphaSources.text(), /Top sources/);
...
assert.match(usersHtml, /viewer-sites-select[^>]+multiple/);
