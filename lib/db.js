...
export class SiteStore {
  constructor(path) {
    ...
    this.insertStatement = this.db.prepare(`
      INSERT INTO events (ts, name, path, referrer, visitor) VALUES (?, ?, ?, ?, ?)`);
    ...
    this.topRefsStatement = this.db.prepare(`
      SELECT coalesce(nullif(referrer,''),'Direct / None') AS label,
        count(*) AS pageviews, count(DISTINCT visitor) AS visitors
      FROM events WHERE ts >= ? AND name = 'pageview'
  ...
  analytics(since) {
    return {
      summary: this.summaryStatement.get(since),
...
export class RisultaDatabase {
  ...
  migrateLegacy() {
    if (Number(this.control.prepare("SELECT count(*) AS n FROM sites").get().n)) return;
    const legacyPath = ["risulta.db", "hutch.db"].map((name) => join(this.dataDir, name)).find(existsSync);
    if (!legacyPath) return;
    let domain = process.env.RISULTA_SITE_DOMAIN || "legacy.local";
    const legacy = new DatabaseSync(legacyPath);
    try {
      const columns = new Set(legacy.prepare("PRAGMA table_info(events)").all().map((column) => column.name));
      if (columns.has("host")) domain = legacy.prepare("SELECT host FROM events WHERE host IS NOT NULL AND host != '' ORDER BY ts DESC LIMIT 1").get()?.host || domain;
      legacy.exec("PRAGMA wal_checkpoint(TRUNCATE)");
    } finally {
      legacy.close();
    }
    const dbName = `${key(12)}.db`;
    renameSync(legacyPath, join(this.sitesDir, dbName));
    this.control.prepare(`
      INSERT INTO sites (name, domain, public_key, db_name, created_at) VALUES (?, ?, ?, ?, ?)`)
      .run(domain, domain, key(), dbName, Math.floor(Date.now() / 1000));
    const site = this.control.prepare("SELECT * FROM sites WHERE db_name = ?").get(dbName);
    this.siteStore(site).close();
  ...
  createUser(email, passwordHash, role = "viewer", siteIds = []) {
    this.control.exec("BEGIN IMMEDIATE");
    try {
      const result = this.control.prepare(`
        INSERT INTO users (email, password_hash, role, created_at) VALUES (?, ?, ?, ?)`)
        .run(email, passwordHash, role, Math.floor(Date.now() / 1000));
