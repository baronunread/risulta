...
const schemaVersion = (db) => Number(db.prepare("PRAGMA user_version").get().user_version);
const checksum = (path) => createHash("sha256").update(readFileSync(path)).digest("hex");

function snapshotFile(directory, path, db, details = {}) {
...
function validSnapshotPath(path) {
  return path === "control.db" || /^sites\/[A-Za-z0-9_-]+\.db$/.test(path);
}
...
export function verifySnapshot(directory) {
  ...
  catch { throw new Error("backup manifest is missing or invalid"); }
  if (manifest?.format !== 1 || !Array.isArray(manifest.databases) || !manifest.databases.length) throw new Error("backup manifest has an unsupported format");
  if (manifest.databases.filter((entry) => entry?.path === "control.db").length !== 1) throw new Error("backup manifest must contain one control database");
  const seen = new Set();
  for (const entry of manifest.databases) {
    if (!validSnapshotPath(entry?.path) || seen.has(entry.path)) throw new Error("backup manifest contains an invalid database path");
    seen.add(entry.path);
    if (!Number.isSafeInteger(entry.bytes) || entry.bytes < 0 || !Number.isInteger(entry.schema_version) || entry.schema_version < 0 || !/^[a-f0-9]{64}$/.test(entry.sha256 || "")) throw new Error(`backup manifest entry is invalid: ${entry.path}`);
    const file = join(directory, entry.path);
    if (!existsSync(file)) throw new Error(`backup is incomplete: ${entry.path} is missing`);
    if (statSync(file).size !== entry.bytes) throw new Error(`backup size check failed: ${entry.path}`);
    if (checksum(file) !== entry.sha256) throw new Error(`backup checksum failed: ${entry.path}`);
  }
  return manifest;
}
...
export class SiteStore {
  constructor(path) {
    ...
    this.summaryStatement = this.db.prepare(`
      WITH scoped AS (
        ...
        FROM events WHERE ts >= ? AND ts < ? AND name = 'pageview'
      )
    ...
    this.byDayStatement = this.db.prepare(`
      ...
      SELECT date(ts, 'unixepoch') AS day, count(*) AS pageviews, count(DISTINCT visitor) AS visitors,
        coalesce(sum(CASE WHEN previous_ts IS NULL OR ts - previous_ts > 1800 THEN 1 ELSE 0 END), 0) AS visits
      FROM scoped
    ...
    this.topMediumsStatement = this.db.prepare(`
      SELECT medium AS label, count(*) AS pageviews, count(DISTINCT visitor) AS visitors
      FROM events WHERE ts >= ? AND ts < ? AND name = 'pageview' AND medium != ''
      GROUP BY medium ORDER BY visitors DESC, pageviews DESC LIMIT 8`);
    this.topCampaignsStatement = this.db.prepare(`
      SELECT campaign AS label, count(*) AS pageviews, count(DISTINCT visitor) AS visitors
      FROM events WHERE ts >= ? AND ts < ? AND name = 'pageview' AND campaign != ''
    ...
    this.goalStatement = this.db.prepare(`
      ...
      FROM events WHERE ts >= ? AND ts < ? AND name = ? AND (? = '' OR path = ?)`);
    this.funnelEventsStatement = this.db.prepare(`
      SELECT visitor, ts, name, path FROM events WHERE ts >= ? AND ts < ? ORDER BY visitor, ts`);
  ...
  insert(event) {
    ...
    this.insertStatement.run(event.ts, event.name, event.path, event.referrer, event.visitor,
      attribution.source, attribution.medium, attribution.campaign, attribution.content, attribution.term, event.value ?? null);
  }
  ...
  analytics(since, goals = [], funnels = [], until = Number.MAX_SAFE_INTEGER) {
    const summary = this.summaryStatement.get(since, until);
    const goalResults = goals.map((goal) => {
      const result = this.goalStatement.get(since, until, goal.event_name, goal.path, goal.path);
      return { ...goal, ...result, conversion_rate: Number(summary.visitors) ? Number(result.unique_conversions) / Number(summary.visitors) : 0 };
    });
    const events = this.funnelEventsStatement.all(since, until);
    const funnelResults = funnels.map((funnel) => {
      const completed = funnel.steps.map(() => new Set());
      const positions = new Map();
      for (const event of events) {
        const position = positions.get(event.visitor) || 0;
        const step = funnel.steps[position];
        if (!step || event.name !== step.event_name || (step.path && event.path !== step.path)) continue;
        completed[position].add(event.visitor);
        positions.set(event.visitor, position + 1);
      }
      return { ...funnel, steps: funnel.steps.map((step, index) => ({ ...step, conversions: completed[index].size, drop_off: index ? completed[index - 1].size - completed[index].size : 0 })) };
    });
    return {
      summary,
      current: Number(this.currentStatement.get().n),
      byDay: this.byDayStatement.all(since, until),
      byHour: this.byHourStatement.all(since, until),
      paths: this.topPathsStatement.all(since, until),
      referrers: this.topRefsStatement.all(since, until),
      mediums: this.topMediumsStatement.all(since, until),
      campaigns: this.topCampaignsStatement.all(since, until),
      goals: goalResults,
      funnels: funnelResults,
    };
  }

  report(since, until, dimension, filters = {}, { limit = 50, offset = 0, sort = "visitors" } = {}) {
    const dimensions = {
    ...
    for (const [name, column] of [["path", "path"], ["campaign", "campaign"], ["event", "name"]]) {
      if (filters[name]) { conditions.push(`${column} = ?`); values.push(filters[name]); }
    }
    if (filters.source) { conditions.push("coalesce(nullif(source,''),'Direct / None') = ?"); values.push(filters.source); }
    if (filters.medium) { conditions.push("coalesce(nullif(medium,''),'None') = ?"); values.push(filters.medium); }
    const where = conditions.join(" AND ");
    const ordering = ["visitors", "pageviews", "value"].includes(sort) ? sort : "visitors";
    const boundedLimit = Math.max(1, Math.min(100, Number(limit) || 50));
    const boundedOffset = Math.max(0, Math.min(10_000, Number(offset) || 0));
    const grouped = `SELECT ${selected.label} AS label, count(*) AS pageviews, count(DISTINCT visitor) AS visitors, coalesce(sum(value), 0) AS value FROM events WHERE ${where} GROUP BY label`;
    const rows = this.db.prepare(`${grouped} ORDER BY ${ordering} DESC, label LIMIT ? OFFSET ?`).all(...values, boundedLimit, boundedOffset);
    const total = Number(this.db.prepare(`SELECT count(*) AS n FROM (${grouped})`).get(...values).n);
    return { dimension, filters, rows, total, limit: boundedLimit, offset: boundedOffset, sort: ordering };
  }
...
export class RisultaDatabase {
  ...
  listFunnels(siteId) {
    const rows = this.control.prepare(`
      SELECT funnels.id AS funnel_id, funnels.name AS funnel_name, funnel_steps.position,
        goals.id AS goal_id, goals.name AS goal_name, goals.event_name, goals.path
      FROM funnels JOIN funnel_steps ON funnel_steps.funnel_id = funnels.id
      JOIN goals ON goals.id = funnel_steps.goal_id
      WHERE funnels.site_id = ? ORDER BY funnels.created_at, funnels.id, funnel_steps.position`).all(siteId);
    const funnels = new Map();
    for (const row of rows) {
      if (!funnels.has(row.funnel_id)) funnels.set(row.funnel_id, { id: row.funnel_id, name: row.funnel_name, steps: [] });
      funnels.get(row.funnel_id).steps.push({ id: row.goal_id, name: row.goal_name, event_name: row.event_name, path: row.path });
    }
    return [...funnels.values()];
  }

  createFunnel(siteId, name, goalIds) {
    this.control.exec("BEGIN IMMEDIATE");
    try {
      const findGoal = this.control.prepare("SELECT id FROM goals WHERE site_id = ? AND id = ?");
      if (goalIds.length < 2 || new Set(goalIds).size !== goalIds.length || goalIds.some((id) => !findGoal.get(siteId, id))) throw new Error("invalid funnel goals");
      const result = this.control.prepare("INSERT INTO funnels (site_id, name, created_at) VALUES (?, ?, ?)")
        .run(siteId, name, Math.floor(Date.now() / 1000));
      const funnelId = Number(result.lastInsertRowid);
      const addStep = this.control.prepare("INSERT INTO funnel_steps (funnel_id, position, goal_id) VALUES (?, ?, ?)");
      goalIds.forEach((goalId, index) => addStep.run(funnelId, index + 1, goalId));
      this.control.exec("COMMIT");
      return funnelId;
    } catch (error) {
      this.control.exec("ROLLBACK");
  ...
  async snapshot(directory) {
    ...
    try {
      ...
      const databases = [snapshotFile(temporary, "control.db", this.control, { kind: "control" })];
      for (const site of sites) {
        const store = this.siteStore(site);
        const path = `sites/${site.db_name}`;
        await backup(store.db, join(temporary, path));
        databases.push(snapshotFile(temporary, path, store.db, { kind: "site", site_id: site.id, site_name: site.name }));
      }
      writeFileSync(join(temporary, "manifest.json"), `${JSON.stringify({ format: 1, created_at: new Date().toISOString(), risulta_version: VERSION, databases }, null, 2)}\n`);
