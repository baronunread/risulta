...
export class SiteStore {
  constructor(path) {
    ...
    this.insertStatement = this.db.prepare(`
      INSERT INTO events (ts, name, path, referrer, visitor, source, medium, campaign, content, term)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
  ...
  insert(event) {
    ...
    this.insertStatement.run(event.ts, event.name, event.path, event.referrer, event.visitor,
      attribution.source, attribution.medium, attribution.campaign, attribution.content, attribution.term);
  }
